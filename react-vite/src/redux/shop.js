...
export const createShop = (name) => async (dispatch) => {
    try {
        ...
        if (response.ok) {
            ...
            throw new Error(errors.errors.shopname);
        }
    } catch (error) {
...
export const modifyShop = (shopId, shopName) => async (dispatch) => {
    try {
        ...
        if (!response.ok) {
            const errors = await response.json();
            const errorMessage = errors.errors && errors.errors.shopname
                ? errors.errors.shopname[0]
                : 'Update shop error occurred from redux';
